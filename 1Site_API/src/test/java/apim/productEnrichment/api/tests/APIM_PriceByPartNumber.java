package apim.productEnrichment.api.tests;
...
public class APIM_PriceByPartNumber extends Base_Class_API implements ITest {
	...
	public void validateGenericErrorCodes(String testname) {
		...
		case "401":
			...
			if (FunLibrary.excelData.get("TestMethodName").contains("Mandatory")) {
				funLibrary.validateJSONPathValue_Equals(response, "errors[0].properties.ParameterName",
						FunLibrary.excelData.get("ParameterName"));
			} else if (FunLibrary.excelData.get("TestMethodName").equals("ImproperFormat")
					|| FunLibrary.excelData.get("TestMethodName").equals("ImproperDataTypes")) {
				funLibrary.validateJSONPathValue_Equals(response, "errors[0].properties.ParameterName",
						FunLibrary.excelData.get("ParameterName"));
				funLibrary.validateJSONPathValue_Equals(response, "errors[0].properties.expectedDataType",
						FunLibrary.excelData.get("ExpectedDataType"));
			}
			break;
		case "404":
	...
	public void validatePartNumber() {
		...
		String[] partnumber = FunLibrary.excelData.get("QueryParameterValue2").split(",");
		...
		RestLibrary restLibrary_SearchIndex = new RestLibrary(FunLibrary.excelData.get("APIM_URI"), searchIndexBaseURI,
				FunLibrary.excelData.get("RequestType"));

		Response searchIndexResponse = restLibrary_SearchIndex.executeAPI();
		restLibrary_SearchIndex.getResponseBody(searchIndexResponse);

		SearchIndexRoot indexResponse = (SearchIndexRoot) restLibrary_SearchIndex.getResponseBody(searchIndexResponse,
				SearchIndexRoot.class);
		Root root = (Root) restLibrary.getResponseBody(apiResponse, Root.class);
		...
		for (i = 0; i < partnumber.length; ++i) {
			...
			PartNumber apiResponseData = root.getPartNumbers().stream()
					.filter(x -> x.getPartNumber().equalsIgnoreCase(partnumbervalue)).findFirst().orElse(null);
			CatalogEntryView searchIndexData = indexResponse.getCatalogEntryView().stream()
					.filter(x -> x.getPartNumber().equalsIgnoreCase(partnumbervalue + "P")).findFirst().orElse(null);
			// Getting PartNumber
			...
			if (partNumber_SI == null || partNumber == null) {
				...
				Assert.assertEquals(partNumber, partNumber_SI,
						"PartNumber value for " + partnumbervalue + " is not matching");
			}
		}
		funLibrary.Assert.assertAll();
	}
	...
	public void validateAttributesOfPartNumber() {
		...
		Root root = (Root) restLibrary_SearchIndex.getResponseBody(apiResponse, Root.class);
		SearchIndexRoot indexResponse = (SearchIndexRoot) restLibrary_SearchIndex.getResponseBody(searchIndexResponse,
				SearchIndexRoot.class);
		...
		for (i = 0; i < partnumber.length; ++i) {
			...
			List<String> excludeFromSubstitutionFlag_SI = searchIndexData.getAttributesMap()
					.geteXCLUDEFROMSUBSTITUTIONFLAG();
			...
			Boolean excludeFromSubstitutionFlag = apiResponseData.getAttributesOfPartNumber()
					.isExcludedFromSubstitution();
			...
			if (barcodeId == null) {
				...
				if (barcodeId_SI == null) {
					...
					Assert.assertEquals(barcodeId_SI, barcodeId,
							"barcodeId for partnumber " + partnumbervalue + " is not matching");
				}
			}
			...
			if (excludeFromSubstitutionFlag == null) {
				...
				if (excludeFromSubstitutionFlag_SI == null) {
					...
					Assert.assertTrue(false,
							"ExcludeFromSubstitution for partnumber " + partnumbervalue + " is not matching");
				} else {
					...
					Assert.assertEquals(excludeFromSubstitutionFlag_SI.contains(excludeFromSubstitutionFlag.toString()),
							true, "ExcludeFromSubstitution for partnumber " + partnumbervalue + " is not matching");
				}
			}
			...
			if (weightedItemIndicator == null) {
				...
				if (weightedItemIndicator_SI == null) {
					...
					Assert.assertTrue(false,
							"WeightedItemIndicator for partnumber " + partnumbervalue + " is not matching");
				} else {
					...
					Assert.assertEquals(weightedItemIndicator_SI.contains(weightedItemIndicator.toString()), true,
							"WeightedItemIndicator for partnumber " + partnumbervalue + " is not matching");
				}
			}
			...
			if (weightedByEach == null) {
				...
				if (weightedByEach_SI == null) {
					...
					Assert.assertEquals(weightedByEach_SI.contains(weightedByEach.toString()), true,
							"WeightedByEach for partnumber " + partnumbervalue + " is not matching");
				}
			}
			...
			if (deliveryRestriction == null) {
				...
				if (deliveryRestriction_SI == null) {
					...
					Assert.assertTrue(false,
							"DeliveryRestriction for partnumber " + partnumbervalue + " is not matching");
				} else {
					...
					Assert.assertEquals(deliveryRestriction_SI, deliveryRestriction,
							"DeliveryRestriction for partnumber " + partnumbervalue + " is not matching");
				}
			}
		}
		...
		for (i = 0; i < partnumber.length; ++i) {
			...
			if (offerPrice == null) {
				...
				if (offerPrice_SI == null) {
					...
					Assert.assertEquals(offerPrice.toString(), offerPrice_SI,
							"OfferPrice for partnumber " + partnumbervalue + " is not matching");
				}
			}
			// validate listPrice
			if (listPrice == null) {
				...
				if (listPrice_SI == null) {
					...
					Assert.assertEquals(listPrice.toString(), listPrice_SI,
							"ListPrice for partnumber " + partnumbervalue + " is not matching");
				}
			}
		}
		funLibrary.Assert.assertAll();
		...
		for (i = 0; i < partnumber.length; ++i) {
			...
			if (redemptionLimitOfRetail == null) {
				...
				if (redemptionLimitOfRetail_SI == null) {
					...
					Assert.assertTrue(false,
							"redemptionLimitOfRetail for partnumber " + partnumbervalue + " is not matching");
				} else {
					...
					Assert.assertEquals(redemptionLimitOfRetail_SI.contains(redemptionLimitOfRetail.toString()), true,
							"redemptionLimitOfRetail for partnumber " + redemptionLimitOfRetail + " is not matching");
				}
			}
			...
			if (redemptionLimitOfPromo == null) {
				...
				if (redemptionLimitOfPromo_SI == null) {
					...
					Assert.assertTrue(false,
							"redemptionLimitOfPromo for partnumber " + partnumbervalue + " is not matching");
				} else {
					...
					Assert.assertEquals(redemptionLimitOfPromo_SI.contains(redemptionLimitOfPromo.toString()), true,
							"redemptionLimitOfPromo for partnumber " + partnumbervalue + " is not matching");
				}
			}
		}
		...
		for (i = 0; i < partnumber.length; ++i) {
			...
			if (liquorAgeRestricted == null) {
				...
				if (liquorAgeRestricted_SI == null) {
					...
					Assert.assertTrue(false,
							"liquorAgeRestricted for partnumber " + partnumbervalue + " is not matching");
				} else {
					...
					Assert.assertEquals(liquorAgeRestricted_SI.contains(liquorAgeRestricted.toString()), true,
							"liquorAgeRestricted for partnumber " + liquorAgeRestricted + " is not matching");
				}
			}
			...
			if (tobaccoAgeRestricted == null) {
				...
				if (tobaccoAgeRestricted_SI == null) {
					...
					Assert.assertTrue(false,
							"tobaccoAgeRestricted for partnumber " + partnumbervalue + " is not matching");
				} else {
					...
					Assert.assertEquals(tobaccoAgeRestricted_SI.contains(tobaccoAgeRestricted.toString()), true,
							"tobaccoAgeRestricted for partnumber " + partnumbervalue + " is not matching");
				}
			}
		}
	...
	public void validateProduct_cooOfPartNumber() {
		...
		for (i = 0; i < partnumber.length; ++i) {
			...
			if (searchIndexData.getCountryOfOrigin().getCoologorq() == null
					&& apiResponseData.getCooOfPartNumber().isCooLogoRequired() == null) {
			...
			} else if (searchIndexData.getCountryOfOrigin().getCoologorq() == null
					|| apiResponseData.getCooOfPartNumber().isCooLogoRequired() == null) {
				testLog.info("COO logo Actual:" + apiResponseData.getCooOfPartNumber().isCooLogoRequired());
				testLog.info("COO logo Expected:" + searchIndexData.getCountryOfOrigin().getCoologorq());
				...
				if (searchIndexData.getCountryOfOrigin().getCoologorq().equalsIgnoreCase("Y")) {
					testLog.info("COO logo Actual:" + apiResponseData.getCooOfPartNumber().isCooLogoRequired());
					testLog.info("COO logo Expected:" + searchIndexData.getCountryOfOrigin().getCoologorq());
					Assert.assertTrue(apiResponseData.getCooOfPartNumber().isCooLogoRequired(),
							"COO logo for partnumber" + partnumbervalue + " is NOT matching");
					...
					Assert.assertFalse(apiResponseData.getCooOfPartNumber().isCooLogoRequired(),
							"COO logo for partnumber" + partnumbervalue + " is NOT matching");
				}
			}

			// Validating COO barcode boolean value
			if (searchIndexData.getCountryOfOrigin().getCoobcrq() == null
					&& apiResponseData.getCooOfPartNumber().isCooBarcodeRequired() == null) {
			...
			} else if (searchIndexData.getCountryOfOrigin().getCoobcrq() == null
					|| apiResponseData.getCooOfPartNumber().isCooBarcodeRequired() == null) {
				testLog.info("COO barcode Actual:" + apiResponseData.getCooOfPartNumber().isCooBarcodeRequired());
				testLog.info("COO barcode Expected:" + searchIndexData.getCountryOfOrigin().getCoobcrq());
				...
				if (searchIndexData.getCountryOfOrigin().getCoobcrq().equalsIgnoreCase("Y")) {
					testLog.info("COO barcode Actual:" + apiResponseData.getCooOfPartNumber().isCooBarcodeRequired());
					testLog.info("COO barcode Expected:" + searchIndexData.getCountryOfOrigin().getCoobcrq());
					Assert.assertTrue(apiResponseData.getCooOfPartNumber().isCooBarcodeRequired(),
							"COO barcode for partnumber" + partnumbervalue + " is NOT matching");
				} else if (searchIndexData.getCountryOfOrigin().getCoobcrq().equalsIgnoreCase("N")) {
					testLog.info("COO barcode Actual:" + apiResponseData.getCooOfPartNumber().isCooBarcodeRequired());
					testLog.info("COO barcode Expected:" + searchIndexData.getCountryOfOrigin().getCoobcrq());
					Assert.assertFalse(apiResponseData.getCooOfPartNumber().isCooBarcodeRequired(),
							"COO barcode for partnumber" + partnumbervalue + " is NOT matching");
				}
			}

			// Validating COO description boolean value
			if (searchIndexData.getCountryOfOrigin().getCoostdsind() == null
					&& apiResponseData.getCooOfPartNumber().isCooDescriptionRequired() == null) {
				testLog.info(
						"COO description Actual:" + apiResponseData.getCooOfPartNumber().isCooDescriptionRequired());
				testLog.info("COO description Expected:" + searchIndexData.getCountryOfOrigin().getCoostdsind());
				Assert.assertTrue(true);
			} else if (searchIndexData.getCountryOfOrigin().getCoostdsind() == null
					|| apiResponseData.getCooOfPartNumber().isCooDescriptionRequired() == null) {
				testLog.info(
						"COO description Actual:" + apiResponseData.getCooOfPartNumber().isCooDescriptionRequired());
				testLog.info("COO description Expected:" + searchIndexData.getCountryOfOrigin().getCoostdsind());
				...
				if (searchIndexData.getCountryOfOrigin().getCoostdsind().equalsIgnoreCase("Y")) {
					testLog.info("COO description Actual:"
							+ apiResponseData.getCooOfPartNumber().isCooDescriptionRequired());
					testLog.info("COO description Expected:" + searchIndexData.getCountryOfOrigin().getCoostdsind());
					Assert.assertTrue(apiResponseData.getCooOfPartNumber().isCooDescriptionRequired(),
							"COO description for partnumber" + partnumbervalue + " is NOT matching");
				...
				if (searchIndexData.getCountryOfOrigin().getCoopercentage() == null
						&& apiResponseData.getCooOfPartNumber().getCooPercentage() == null) {
				...
				} else if (searchIndexData.getCountryOfOrigin().getCoopercentage() == null
						|| apiResponseData.getCooOfPartNumber().getCooPercentage() == null) {
					testLog.info("COO percentage Actual:" + apiResponseData.getCooOfPartNumber().getCooPercentage());
					testLog.info("COO percentage Expected:" + searchIndexData.getCountryOfOrigin().getCoopercentage());
					...
					if (apiResponseData.getCooOfPartNumber().getCooPercentage() == 0) {
						...
						if (searchIndexData.getCountryOfOrigin().getCoopercentage() == "N/A"
								|| searchIndexData.getCountryOfOrigin().getCoopercentage() == null) {
							testLog.info(
									"COO percentage Actual:" + apiResponseData.getCooOfPartNumber().getCooPercentage());
							testLog.info("COO percentage Expected:"
									+ searchIndexData.getCountryOfOrigin().getCoopercentage());
						}
					} else {
						testLog.info(
								"COO percentage Actual:" + apiResponseData.getCooOfPartNumber().getCooPercentage());
						testLog.info(
								"COO percentage Expected:" + searchIndexData.getCountryOfOrigin().getCoopercentage());
						Assert.assertEquals(apiResponseData.getCooOfPartNumber().getCooPercentage().toString(),
								searchIndexData.getCountryOfOrigin().getCoopercentage(),
								"COO percentage for partnumber " + partnumbervalue + " is not matching");
					}
				}

				// Validating COO description value
				if (searchIndexData.getCountryOfOrigin().getCoostmt() == null
						&& apiResponseData.getCooOfPartNumber().getCooDescription() == null) {
					...
					Assert.assertTrue(true);
				} else if (searchIndexData.getCountryOfOrigin().getCoostmt() == null
						|| apiResponseData.getCooOfPartNumber().getCooDescription() == null) {
					testLog.info("COO description Actual:" + apiResponseData.getCooOfPartNumber().getCooDescription());
					...
					testLog.info("COO description Actual:" + apiResponseData.getCooOfPartNumber().getCooDescription());
					testLog.info("COO description Expected:" + searchIndexData.getCountryOfOrigin().getCoostmt());
					Assert.assertEquals(apiResponseData.getCooOfPartNumber().getCooDescription(),
							searchIndexData.getCountryOfOrigin().getCoostmt(),
							"COO description for partnumber " + partnumbervalue + " is not matching");
				}

				// Validating COO statement value
				if (searchIndexData.getCountryOfOrigin().getCoostmt() == null
						&& apiResponseData.getCooOfPartNumber().getCooStatement() == null) {
					...
					testLog.info("COO statement Actual:" + apiResponseData.getCooOfPartNumber().getCooStatement());
					testLog.info("COO statement Expected:" + searchIndexData.getCountryOfOrigin().getCoostmt());
					Assert.assertEquals(apiResponseData.getCooOfPartNumber().getCooStatement(),
							searchIndexData.getCountryOfOrigin().getCoostmt(),
							"COO statement for partnumber " + partnumbervalue + " is not matching");
				}

				// Validating COO country name
				if (searchIndexData.getCountryOfOrigin().getCoocountry() == null
						&& apiResponseData.getCooOfPartNumber().getCooCountry() == null) {
					...
					Assert.assertTrue(true);
				} else if (searchIndexData.getCountryOfOrigin().getCoocountry() == null
						|| apiResponseData.getCooOfPartNumber().getCooCountry() == null) {
					testLog.info("COO statement Actual:" + apiResponseData.getCooOfPartNumber().getCooCountry());
					testLog.info("COO statement Expected:" + searchIndexData.getCountryOfOrigin().getCoocountry());
					Assert.assertTrue(false, "COO statement for partnumber " + partnumbervalue + " is not matching");
					...
					testLog.info("COO statement Actual:" + apiResponseData.getCooOfPartNumber().getCooCountry());
					testLog.info("COO statement Expected:" + searchIndexData.getCountryOfOrigin().getCoocountry());
					Assert.assertEquals(apiResponseData.getCooOfPartNumber().getCooCountry(),
							searchIndexData.getCountryOfOrigin().getCoocountry(),
							"COO statement for partnumber " + partnumbervalue + " is not matching");
				}
			}
		}
	...
	public void validateProduct_unitPriceDisplayOfPartNumber() {
		...
		for (i = 0; i < partnumber.length; ++i) {
			...
			if (searchIndexData.getUnitPrice() == null
					&& apiResponseData.getPriceDisplayOfPartNumber().getUnitPriceDisplay() == null) {
				testLog.info(
						"API unit price Actual:" + apiResponseData.getPriceDisplayOfPartNumber().getUnitPriceDisplay());
				testLog.info("SI unit price Expected:" + searchIndexData.getUnitPrice());
				Assert.assertTrue(true);
			} else if (searchIndexData.getUnitPrice() == null
					|| apiResponseData.getPriceDisplayOfPartNumber().getUnitPriceDisplay() == null) {
				testLog.info(
						"API unit price Actual:" + apiResponseData.getPriceDisplayOfPartNumber().getUnitPriceDisplay());
				testLog.info("SI unit price Expected:" + searchIndexData.getUnitPrice());
				...
				testLog.info(
						"API unit price Actual:" + apiResponseData.getPriceDisplayOfPartNumber().getUnitPriceDisplay());
				testLog.info("SI unit price Expected:" + searchIndexData.getUnitPrice());
				Assert.assertEquals(apiResponseData.getPriceDisplayOfPartNumber().getUnitPriceDisplay(),
						searchIndexData.getUnitPrice(),
						"Unit price for partnumber " + partnumbervalue + " is not matching");
			}
		}
		funLibrary.Assert.assertAll();
	}
	...
	public void validateProduct_temporarilyUnavailable() {
		...
		for (i = 0; i < partnumber.length; ++i) {
			...
			if (searchIndexData.isAvailable() == null && apiResponseData.isTemporarilyUnavailable() == null) {
				...
				testLog.info("SI product available Expected:" + searchIndexData.isAvailable());
				Assert.assertTrue(false,
						"Temparary unavailable for partnumber " + partnumbervalue + " is not matching");
			} else {
				if (apiResponseData.isTemporarilyUnavailable() == true && searchIndexData.isAvailable() == false) {
					testLog.info(
							"API product temporary unavailable Actual:" + apiResponseData.isTemporarilyUnavailable());
					testLog.info("SI product available Expected:" + searchIndexData.isAvailable());
				} else if (apiResponseData.isTemporarilyUnavailable() == false
						&& searchIndexData.isAvailable() == null) {
					testLog.info(
							"API product temporary unavailable Actual:" + apiResponseData.isTemporarilyUnavailable());
					testLog.info("SI product available Expected:" + searchIndexData.isAvailable());
				} else {
					...
					testLog.info("SI product available Expected:" + searchIndexData.isAvailable());
					Assert.assertTrue(false,
							"API and SI temprary unavailable for " + partnumbervalue + " is NOT matching");
				}
			}
		}
		funLibrary.Assert.assertAll();
	...
	public void validateTicketType_Special(String testName) {
		...
		for (i = 0; i < partnumber.length; ++i) {
			...
			String listPrice_SI = format.format(Double.parseDouble(searchIndexData.getPrice().getListPrice()));
			String offerPrice_SI = searchIndexData.getPrice().getOfferPrice();
			...
			List<String> hideColesPromotionIconFlag_SI = searchIndexData.getAttributesMap()
					.gethIDECOLESPROMOTIONICONFLAG();
			...
			Boolean hideColesPromotionIconFlag_API = apiResponseData.getAttributesOfPartNumber()
					.ishideColesPromotionIconFlag();
			String ticketType_API = apiResponseData.getTicketTypeOfPartNumber().getTicketType();
			String offerPrice_API = format.format(apiResponseData.getPricesOfPartNumber().getOfferPrice());
			String listPrice_API = format.format(apiResponseData.getPricesOfPartNumber().getListPrice());
			...
			funLibrary.validate_Equals("GrahicCode", graphicCode,
					FunLibrary.excelData.get("TicketType") + "_" + onlinePromo);
			// Validating hideProductSavingsFlag boolean value
			funLibrary.validate_Contains("HideColesPromotionIconFlag",
					hideColesPromotionIconFlag_SI, hideColesPromotionIconFlag_API.toString());
			// Validating hideProductSavingsFlag boolean value
			funLibrary.validate_Contains("Hide_ProductSavingsFlag", 
					hideProductSavingsFlag_SI,hideProductSavingsFlag_API.toString());
		}
		funLibrary.Assert.assertAll();
	}
	...
	public void validateTicketType_Online_Special(String testName) {
		...
		for (i = 0; i < partnumber.length; ++i) {
			...
			DecimalFormat format = new DecimalFormat("0.##");
			String listPrice_SI = format.format(Double.parseDouble(searchIndexData.getPrice().getListPrice()));
			String offerPrice_SI = format.format(Double.parseDouble(searchIndexData.getPrice().getOfferPrice()));
			double discount_Price = Double.parseDouble(listPrice_SI) - Double.parseDouble(offerPrice_SI);
			String secondShout_SI = "save $" + format.format(discount_Price);
	...
	public void validateTicketType_Online_Special40() {
		...
		for (i = 0; i < partnumber.length; ++i) {
			...
			testLog.info("-----------Validating Ticket Type - S40 : " + partnumbervalue + " -----------");
	...
	public void validateTicketType_Online_Special50() {
		...
		for (i = 0; i < partnumber.length; ++i) {
			...
			testLog.info("-----------Validating Ticket Type - S50 : " + partnumbervalue + " -----------");
	...
	public void validateTicketType_New() {
		...
		for (i = 0; i < partnumber.length; ++i) {
			...
			if (graphicCode.equals(FunLibrary.excelData.get("TicketType") + "_" + onlinePromo)) {
				...
				testLog.info("GraphicCode API Response Expected:" + FunLibrary.excelData.get("TicketType") + "_"
						+ onlinePromo);
			} else {
				testLog.error("GraphicCode API Response Actual:" + ticketType);
				testLog.error("GraphicCode API Response Expected: " + FunLibrary.excelData.get("TicketType") + "_"
						+ onlinePromo);
	...
	public void validateTicketType_EveryDay() {
		...
		for (i = 0; i < partnumber.length; ++i) {
			...
			testLog.info(
					"-----------Validating Ticket Type - EveryDay for PartNumber: " + partnumbervalue + " -----------");
	...
	public void validateTicketType_Flybuy(String testName) {
		...
		for (i = 0; i < partnumber.length; ++i) {
			...
			testLog.info(
					"-----------Validating Ticket Type - FlyBuy for PartNumber: " + partnumbervalue + " -----------");
			...
			// validating graphicCode
			funLibrary.validate_Equals("GraphicCode", FunLibrary.excelData.get("TicketType") + "_" + onlinePromo,
					graphicCode);
	...
	public void validateTicketType_MultiSaveMultiSKU() {
		...
		for (i = 0; i < partnumber.length; ++i) {
			...
			funLibrary.validate_Equals("GraphicCode", FunLibrary.excelData.get("TicketType") + "_" + onlinePromo,
					graphicCode);
			// validating 1st Shout
			funLibrary.validate_Contains("1stShout",
					"Any " + promoMinQty + " $" + promoDesc.substring(promoDesc.lastIndexOf("$") + 1), _1stShout);
			// validating promoType
			funLibrary.validate_Equals("PromoType", promoType_SI, promoType);
			// validating promoDesc
			funLibrary.validate_Equals("PromoDesc", promoDesc_SI, promoDesc);
