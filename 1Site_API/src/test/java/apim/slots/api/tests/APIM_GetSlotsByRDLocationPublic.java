...
public class APIM_GetSlotsByRDLocationPublic extends Base_Class_API implements ITest{
	...
	public void validateremoteDeliveryPartnerDetails() {
      ...
      response = Payload.getSlotByRDLocationDate_Public_APIM("", "", "", authorization, Subscription_Key,storeId, collectionPointId, date, date, "ALL");
		restLibrary.getResponseBody(response);
		funLibrary.validateStatusCode(response, FunLibrary.excelData.get("StatusCode"));
	...
	public void validateNoSlotsAvailableForSelected3PL() throws SQLException {
		...
		String collectionPointId = FunLibrary.excelData.get("collectionPointId");
		String Subscription_Key= FunLibrary.excelData.get("Header1_Value");
		System.out.println(Subscription_Key);
		Response response = null;
		DatabaseUtilities dbUtil_DM = new DatabaseUtilities("DM");
		int i,count=0;
		int maxNoOfDays=6;
		for (i = 1; i <= maxNoOfDays;) {
			...
			if (count == 0) {
				response = Payload.getSlotByRDLocationDate_Public_APIM("", "", "", authorization, Subscription_Key,
						storeId, collectionPointId, date, date, "ALL");
				restLibrary.getResponseBody(response);
				funLibrary.validateStatusCode(response, FunLibrary.excelData.get("StatusCode"));
