...
public class UserAddressInterstitialPageTests{
	...
	public void inValidURL_MissingDestination() throws Exception {
		driver.get(
				"https://wcssitint.cmltd.net.au:27901/online/COLRSSSOAuthenticateView?storeId=20501&dest=address-add&channel=sia&col-token="
						+ colToken + "&cid=1234567890");
		Thread.sleep(1000);
		Assert.assertEquals("Something went wrong",driver.findElement(By.xpath("//*[@class='error-heading']")).getText().trim());
	}
	
	@Test(description = "Validate that system naviagte to user add address page")
	public void validURL_UserAddAddress() throws Exception {
		driver.get(
				"https://wcssitint.cmltd.net.au:27901/online/COLRSSSOAuthenticateView?storeId=20501&dest=address-add&channel=sia&col-token="+colToken+"&cid=1234567890&adobe_mc=12345abcde67890#qwert");
		...
		Assert.assertEquals("/a/casuarina/address-add", actual);

	}

	@Test(description = "Validate that system naviagte to user update address page")
	public void validURL_UserUpdateAddress() throws Exception {
		driver.get(
				"https://wcssitint.cmltd.net.au:27901/online/COLRSSSOAuthenticateView?storeId=20501&dest=address-update&channel=sia&col-token="+colToken+"&addressId=59150544&cid=1234567890&adobe_mc=12345abcde67890#qwert");
		Thread.sleep(1000);
		String url = driver.getCurrentUrl();
		URL aURL = new URL(url);
		String actual = aURL.getPath();
		Assert.assertEquals("/a/casuarina/address-update", actual);
