...
public class WCS_GetSlotsByRDLocationPrivate extends Base_Class_API implements ITest{
	...
	public void validateslotBaggingBitFlag() {
		...
		String user_Jwt_Token = FunLibrary.excelData.get("Header1_Value");
		...
		response = Payload.getSlotByRDLocationDate_Private_WCS("", "", "", accessToken,user_Jwt_Token, storeId, collectionPointId, date, date, "ALL");
		restLibrary.getResponseBody(response);
		funLibrary.validateStatusCode(response, FunLibrary.excelData.get("StatusCode"));
	...
	@Test(description = "AC03- Verify there should be 204 no content response when there are no slot available")
	public void validateWhenNoSlotsAvailable() throws SQLException {
		...
		for (i = 1; i <= maxNoOfDays;) {
			...
			if (count == 0) {
				response = Payload.getSlotByRDLocationDate_Private_WCS("", "", "", accessToken, user_Jwt_Token, storeId,
						collectionPointId, date, date, "ALL");
				restLibrary.getResponseBody(response);
				funLibrary.validateStatusCode(response, FunLibrary.excelData.get("StatusCode"));
