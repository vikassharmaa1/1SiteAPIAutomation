package ps.getOrderDetails.api.tests;
...
public class GetOrderDetails extends Base_Class_API implements ITest {
	RestLibrary restLibrary;
	Response response;

	public void getOrderDetails(String orderId) {
		restLibrary = new RestLibrary();
		response = APILibrary.getOrderDetail_PS(Base_Class_API.BaseURI, FunLibrary.excelData.get("BasePath"),
				FunLibrary.excelData.get("RequestType"), FunLibrary.excelData.get("Header_authorization"),
				FunLibrary.excelData.get("Header_userauthorization"), getOrderId(orderId),
	...
	@Test(description = "Validate order submit and verify Get order Detail response")
	public void validateOrderDetails() throws IOException {
		...
		restLibrary.getResponseBody(APILibrary.addItem_PS("", "", "", FunLibrary.excelData.get("Header_authorization"),
				FunLibrary.excelData.get("Header_userauthorization"), "0404", "3967235", "2", "false"));

		// Localizing to HD
		restLibrary.getResponseBody(
				APILibrary.localizationBySuburbPostCode_PS("", "", "", FunLibrary.excelData.get("Header_authorization"),
						FunLibrary.excelData.get("Header_userauthorization"), "20509", "3121", "RICHMOND"));
		restLibrary.getResponseBody(
				APILibrary.localizationBySuburbPostCode_PS("", "", "", FunLibrary.excelData.get("Header_authorization"),
						FunLibrary.excelData.get("Header_userauthorization"), "20510", "0820", "Bayview"));
