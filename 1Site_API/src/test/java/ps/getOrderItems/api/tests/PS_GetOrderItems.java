package ps.getOrderItems.api.tests;
...
public class PS_GetOrderItems  extends Base_Class_API implements ITest {
	Response response;
	...
	public void validateOrderNotAssociatedwithMyAccount() {
		response = 	APILibrary.getOrderItems_PS("", "", "", FunLibrary.excelData.get("Header_UserAuthorization"),
	...
	public void validateOrderItems() {
		...
		response = 	APILibrary.getOrderItems_PS("", "", "", FunLibrary.excelData.get("Header_UserAuthorization"),FunLibrary.excelData.get("Header_Authorization"), "0404", "1", db_order);
		funLibrary.validateStatusCode(response, FunLibrary.excelData.get("StatusCode"));
		System.out.println("final"+"" +db_order);
	...
	public void validateOrderItemsLiquorTobbaco() {
		response = 	APILibrary.getOrderItems_PS("", "", "", FunLibrary.excelData.get("Header_UserAuthorization"),FunLibrary.excelData.get("Header_Authorization"), "0404", "1", FunLibrary.excelData.get("orderId"));
	...
	@Test( description = "AC01- Verify orders items details for HD Order")
	public void validateOrderItemsforHDOrders() {
		DatabaseUtilities dbUtil = new DatabaseUtilities("SIT");
	...
	/*@Test( description = "AC01- Verify orders items details for CC Order")
	public void validateOrderItemsforCCOrders() {
		DatabaseUtilities dbUtil = new DatabaseUtilities("SIT");
		String db_order = dbUtil.getValues("orders", "ORDERS_ID","member_id = (select users_id from users where field1='" + FunLibrary.excelData.get("UserName")+ "') and STATUS='M'  order by LASTUPDATE asc" );
		response = 	APILibrary.getOrderItems_PS("", "", "", FunLibrary.excelData.get("Header_UserAuthorization"),FunLibrary.excelData.get("Header_Authorization"), "0404", "1", db_order);
		funLibrary.validateStatusCode(response, FunLibrary.excelData.get("StatusCode"));
